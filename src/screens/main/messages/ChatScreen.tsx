...
const ChatScreen: React.FC<ChatScreenProps> = ({ navigation, route }) => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      Alert.alert(t('messages.chat.loadError'), t('messages.chat.loadErrorMessage'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      Alert.alert(t('messages.chat.sendError'), t('messages.chat.sendErrorMessage'));
  ...
  const formatDate = (dateString: string) => {
    ...
    if (date.toDateString() === today.toDateString()) {
      return t('messages.chat.today');
    } else if (date.toDateString() === yesterday.toDateString()) {
      return t('messages.chat.yesterday');
    } else {
      const locale = i18n.language === 'en' ? 'en-US' : 'tr-TR';
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        >
          <TextInput
            style={styles.input}
            placeholder={t('messages.chat.typeMessagePlaceholder')}
